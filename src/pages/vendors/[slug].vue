<template>
  <div class="font-sans bg-white text-slate-900 min-h-screen selection:bg-rose-200 selection:text-rose-900 relative overflow-x-hidden">

    <header class="fixed top-0 w-full z-50 bg-white border-b border-slate-100 transition-all duration-300">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        <NuxtLink to="/" class="flex items-center gap-3 group">
          <div class="h-9 w-9 bg-gradient-to-br from-rose-500 to-rose-600 text-white font-serif font-bold flex items-center justify-center rounded-xl shadow-md shadow-rose-200 group-hover:scale-105 transition-transform">SB</div>
          <span class="font-serif font-bold text-lg tracking-tight hidden sm:block text-slate-900">Stage &amp; Bloom</span>
        </NuxtLink>

        <nav class="flex items-center gap-6 text-sm font-medium">
          <NuxtLink to="/vendors/search" class="text-slate-600 hover:text-rose-600 transition-colors flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-full hover:bg-slate-100">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <span class="hidden sm:inline">Explore Vendors</span>
    ...
    <main class="pt-24 pb-24 relative">
      ...
      <section v-else-if="error || !vendor" class="min-h-[50vh] flex flex-col items-center justify-center text-center px-4">
      ...
      <div v-else class="animate-fade-in max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <section class="mb-10">
          <div class="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
            <div class="flex-1">
              <div class="flex flex-wrap items-center gap-3 mb-4 text-sm">
                 <span class="px-3 py-1 rounded-full bg-rose-50 text-rose-700 text-xs font-bold uppercase tracking-wider border border-rose-100 shadow-sm">
                      {{ vendor.vendor_categories?.name || 'Vendor' }}
                </span>
                <span v-if="vendor.is_verified" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gradient-to-r from-emerald-50 to-teal-50 text-emerald-700 text-xs font-bold uppercase tracking-wider border border-emerald-100 shadow-sm">
                    <svg class="w-3.5 h-3.5 text-emerald-500" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" d="M8.603 3.799A4.49 4.49 0 0112 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 013.498 1.307 4.491 4.491 0 011.307 3.497A4.49 4.49 0 0121.75 12a4.49 4.49 0 01-1.549 3.397 4.491 4.491 0 01-1.307 3.497 4.491 4.491 0 01-3.497 1.307A4.49 4.49 0 0112 21.75a4.49 4.49 0 01-3.397-1.549 4.49 4.49 0 01-3.498-1.306 4.491 4.491 0 01-1.307-3.498A4.49 4.49 0 012.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 011.307-3.497 4.491 4.491 0 013.497-1.307zm4.45 10.303a.75.75 0 000-1.06l-3-3a.75.75 0 10-1.06 1.06l2.47 2.47 5.47-5.47a.75.75 0 00-1.06-1.06l-6 6z" clip-rule="evenodd" /></svg>
                    Verified
                 </span>
              </div>
              <h1 class="font-serif text-4xl sm:text-5xl md:text-6xl font-bold text-slate-900 mb-4 leading-tight">{{ vendor.display_name }}</h1>
              ...
              <div class="flex flex-wrap items-center gap-4 text-sm text-slate-600 font-medium">
                ...
                <a v-if="vendor.instagram_url" :href="vendor.instagram_url" target="_blank" class="flex items-center gap-1.5 hover:text-rose-600 transition-colors bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
            ...
            <div class="hidden md:flex items-center gap-3">
              <button class="p-2.5 rounded-full bg-white border border-slate-200 text-slate-500 hover:text-rose-600 hover:border-rose-200 hover:bg-rose-50 transition-all shadow-sm">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-4 gap-3 h-[320px] sm:h-[450px] rounded-3xl overflow-hidden mb-10 shadow-xl shadow-slate-200/40 border border-slate-100 bg-white">
            <div class="sm:col-span-2 sm:row-span-2 relative bg-slate-50 group cursor-pointer" @click="setActiveImage(heroImage)">
              <img :src="heroImage" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Main view" />
            </div>
            <div v-for="(photo, index) in galleryPhotos.slice(0, 2)" :key="photo.id" class="relative bg-slate-50 hidden sm:block group cursor-pointer" @click="setActiveImage(photo.image_url)">
              <img :src="photo.image_url" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Detail view" />
            </div>
            <div class="hidden sm:grid grid-rows-2 gap-3">
              <div v-if="galleryPhotos[2]" class="relative bg-slate-50 group cursor-pointer" @click="setActiveImage(galleryPhotos[2].image_url)">
                <img :src="galleryPhotos[2].image_url" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Detail view" />
              </div>
              <div v-if="galleryPhotos[3]" class="relative bg-slate-50 group cursor-pointer" @click="setActiveImage(galleryPhotos[3].image_url)">
                <img :src="galleryPhotos[3].image_url" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Detail view" />
                <div v-if="galleryPhotos.length > 4" class="absolute inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center text-white font-medium text-sm transition-colors hover:bg-slate-900/80">
                  More photos
                </div>
              </div>
            </div>
        ...
        <div class="flex flex-col-reverse lg:grid lg:grid-cols-3 gap-8 lg:gap-12 relative">
          ...
          <div class="lg:col-span-2 space-y-10">
            ...
            <div v-if="rates.length">
              ...
              <div class="space-y-4">
                <div v-for="r in rates" :key="r.id" class="relative bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:border-rose-200 hover:shadow-md transition-all group overflow-hidden">
                  <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <h4 class="font-bold text-slate-900 text-lg group-hover:text-rose-700 transition-colors">{{ r.service_name || 'Service Package' }}</h4>
                    ...
                    <div class="text-right">
                      ...
                      <p class="text-2xl font-serif font-bold text-slate-900" v-if="r.starting_from_amount">GH₵ {{ r.starting_from_amount.toLocaleString() }}</p>
          ...
          <div class="lg:col-span-1">
            <div class="lg:sticky lg:top-24 space-y-6">
              ...
              <div class="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/20 p-6 relative overflow-hidden">
                ...
                <div class="flex items-baseline justify-between mb-8 pb-6 border-b border-slate-100">
                  ...
                  <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50 px-2 py-1 rounded">Starting</span>
                </div>
...
// Types
type VendorRate = { id: string; service_name: string | null; pricing_model: string | null; starting_from_amount: number | null }
